...
public class TransformSync : MonoBehaviour, IEntangleSync
{
    [Range(0, 2)]
    ...
    public FNVector3 Position => networkEntity.Position;
    ...
    public void Init(NetworkEntity networkEntity, System.Numerics.Vector3 _position = default, System.Numerics.Vector4 _rotation = default, System.Numerics.Vector3 _scale = default)
    {
        ...
        if (this.networkEntity.EntityType == EntityType.Player)
        {
            if (this.networkEntity.IsMine)
            {
                ...
                this.networkEntity.SetNetProperty<FNVector4>(244, rotation, FigNet.Core.DeliveryMethod.Unreliable);
            }
            else
            {
                this.networkEntity.GetNetProperty<FNVector4>(244, FigNet.Core.DeliveryMethod.Unreliable, (rot) =>
                {
                    rotation = rot;
            ...
            this.networkEntity.GetNetProperty<FNVector3>(255, FigNet.Core.DeliveryMethod.Unreliable, (pos) =>
            {
                if (this.networkEntity.Position == default) this.networkEntity.Position = new FNVector3();
                this.networkEntity.Position.OnValueChange += Position_OnValueChange;
        ...
        else
        {
            ...
            this.networkEntity.GetNetProperty<FNVector3>(255, FigNet.Core.DeliveryMethod.Unreliable, (pos) =>
            {
                this.networkEntity.Position = pos;
            ...
            if (this.networkEntity.Position == default)
            {
                itemPosition.x = _position.X;
                itemPosition.y = _position.Y;
                itemPosition.z = _position.Z;
                this.networkEntity.Position = new FNVector3()
                {
                ...
                this.networkEntity.SetNetProperty<FNVector3>(255, this.networkEntity.Position, FigNet.Core.DeliveryMethod.Unreliable);
                this.networkEntity.Position.Value = new FNVec3(itemPosition.x, itemPosition.y, itemPosition.z);
                this.networkEntity.Position.OnValueChange += Position_OnValueChange;
            }
    ...
    private void SendMoveUpdate()
    {
        ...
        if (networkEntity.Position != null)
            networkEntity.Position.SetValue(posCache);// new FNVec3(transform.position.x, transform.position.y, transform.position.z);//
        else
